...
export default function Layout({ children }: { children: React.ReactNode }) {
  const { studentName, studentId, role, logout, invoices, declarations } = useApp();
  ...
  return (
    <div className="flex h-screen bg-slate-50">
      ...
      >
        ...
        {studentId === "020230884" && (
          <a
            href="https://xyiu5qejspbrq.ok.kimi.link/#/submission"
            target="_blank"
            rel="noopener noreferrer"
            title="作业提交情况"
            className="flex items-center gap-3 px-4 py-2.5 text-sm text-emerald-100/90 hover:text-white hover:bg-emerald-600/30 active:bg-emerald-600/40 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-200/55 focus-visible:ring-inset"
          >
            <svg className="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden>
            ...
            {!collapsed && <span className="truncate">作业提交情况</span>}
          </a>
        )}
